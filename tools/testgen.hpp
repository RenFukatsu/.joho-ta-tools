...
bool compile_file(std::string fname) {
    ...
    if (ext == ".c") {
        is_success = !system(
            ("/usr/bin/gcc -DONLINE_JUDGE -O2 -w -fmax-errors=3 -std=c11 src/" + fname + " -lm -o a.out").c_str());
    } else if (ext == ".cpp") {
        is_success = !system(
            ("/usr/bin/g++ -DONLINE_JUDGE -O2 -w -fmax-errors=3 -std=c++14 src/" + fname + " -lm -o a.out").c_str());
    } else {
        ...
        while (iss >> type) {
            ...
            if (type == "i") {
                ...
                if (smin.length() == 1 && smax.length() == 1) {
                    ...
                    std::cout << "\033[m031Even though char was asked for, the number of characters is not one.\033[m"
                              << std::endl;
                    exit(EXIT_FAILURE);
                }
                std::uniform_int_distribution<int> dist(min, max);
        ...
        if (s.find("#include") != std::string::npos) {
            ...
            if (subs.find("lf") != std::string::npos) {
                ...
                ofs << str_space << "fscanf(user_output, \"%lf\", &user_val" << std::to_string(num) << ");"
                    << std::endl;
            } else if (subs.find("f") != std::string::npos) {  // st < s.find("f") && s.find("f") < gl
            ...
            } else if (subs.find("c") != std::string::npos) {  // st < s.find("c") && s.find("c") < gl
            ...
            } else if (subs.find("s") != std::string::npos) {  // st < s.find("s") && s.find("s") < gl
            ...
            ofs << str_space << "if(!equiv(" << val << ", user_val" << std::to_string(num) << ")) return WA;"
                << std::endl;
            num++;
...
void folderize(std::string fname) {
    ...
    std::string name = fname.substr(0, idx_ext);
    system(
        ("mkdir -p testcases/" + name + " && mv *.in testcases/" + name + "/ && mv *.out testcases/" + name).c_str());
